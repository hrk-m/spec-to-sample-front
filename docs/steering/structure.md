...
| `group-detail`        | 実装済み（`GroupDetailPage` + `GroupDetailSheet` + `GroupDetailView` + `GroupDetailContent` + `EditGroupDialog` + `DeleteGroupDialog` + `AddMemberSheet` + `MemberDetailSheet` + `MemberList` + `AddSubgroupSheet` + `SubgroupList` コンポーネント、`api/fetch-group.ts`、`api/fetch-group-members.ts`、`api/fetch-groups.ts`（Sheet 用全グループ一覧取得。`GroupSummary` / `FetchGroupsForSheetResponse` 型を export。同一レイヤークロスインポートを避けるため `pages/home/api/fetch-groups.ts` とは独立して実装）、`api/update-group.ts`（PUT リクエスト）、`api/delete-group.ts`（DELETE リクエスト。`apiFetch<void>` を使用し 204 No Content を受け取る）、`api/fetch-non-members.ts`（`GET /api/v1/groups/:id/non-members`。`FetchNonMembersParams` / `NonMembersResponse` 型を export）、`api/add-group-members.ts`（`POST /api/v1/groups/:id/members`。`AddGroupMembersParams` / `AddGroupMembersResponse` 型を export）、`api/delete-group-members.ts`（`DELETE /api/v1/groups/:id/members`。`DeleteGroupMembersParams` 型を export。リクエストボディ `{ user_ids: number[] }` で複数メンバーを一括削除。`apiFetch<void>` を使用）、`api/add-subgroup.ts`（`POST /api/v1/groups/:id/subgroups`。`AddSubgroupParams` / `AddSubgroupResponse` 型を export。リクエストボディ `{ child_group_id: number }`）、`model/group-detail.ts`（`SubgroupSummary` 型（`id`, `name`, `description`, `member_count`）・`GroupDetail` 型（`subgroups: SubgroupSummary[]` フィールドを含む）・`UserSummary` 型・`MembersResponse` 型を定義）、`model/group-update.ts`（`UpdateGroupRequest` 型）、`model/group-detail-state.ts`（`useGroupDetail` フック・`clearGroupDetailCache` 関数を export。`refetch` はキャッシュから対象 ID を削除して再フェッチする。`subgroups: SubgroupSummary[]`（`group?.subgroups ?? []`）も返す）、`model/useUpdateGroup.ts`（`useUpdateGroup` フック）、`model/useDeleteGroup.ts`（`useDeleteGroup` フック）、`model/member-list.ts`（`useMemberList` フック・`FETCH_LIMIT` 定数（100）・`clearMemberListCache` 関数を export。クライアントキャッシュ + IntersectionObserver 無限スクロール + 300ms デバウンス検索）、`model/useNonMemberList.ts`（`useNonMemberList` フック・`FETCH_LIMIT` 定数（100）・`clearNonMemberListCache(groupId?: number)` 関数を export。引数なしで全キャッシュクリア、`groupId` を渡すと対象グループのキャッシュのみ削除する。クライアントキャッシュ + IntersectionObserver 無限スクロール + 300ms デバウンス検索）、`MemberList.styles.ts`（テーブル用スタイル定数: `searchSection`, `searchField`, `searchFieldIcon`, `tableRoot`（`tableLayout: "fixed"` 指定）, `tableHeader`, `tableHeaderCell`, `tableHeaderCellUuid`, `tableHeaderCellCheckbox`, `headerCheckboxInput`, `tableRow`, `tableRowLast`, `tableCellId`, `tableCellName`, `tableCellCheckbox`, `skeletonRow`, `skeletonCell`, `skeletonLine`, `errorText`, `emptyText` 等。各テキストセルは `whiteSpace: "nowrap"`, `overflow: "hidden"`, `textOverflow: "ellipsis"` で長テキストを省略表示）、`AddMemberSheet.styles.ts`（テーブル用スタイル定数: `searchSection`, `searchField`, `searchFieldIcon`, `tableRoot`（`tableLayout: "fixed"` 指定）, `tableHeader`, `tableHeaderCell`, `tableHeaderCellCheckbox`, `tableHeaderCellUuid`, `tableRow`, `tableRowLast`, `tableCellCheckbox`, `tableCellUuid`, `tableCellName`, `errorText`, `emptyText`, `skeletonRow`, `skeletonCell`, `skeletonLine` 等。各テキストセルは `whiteSpace: "nowrap"`, `overflow: "hidden"`, `textOverflow: "ellipsis"` で長テキストを省略表示）、`GroupDetailPage.styles.ts`、テスト）。`MemberList` はネイティブ `<table>` で □選択 / uuid / 姓名 の 3 列を表示し、アバターアイコンは使用しない。`AddMemberSheet` のユーザーリストはネイティブ `<table>` で □選択 / uuid / 姓名 の 3 列を表示し、アバターアイコンは使用しない。`SubgroupList` はカード形式でサブグループ名・説明を一覧表示し、各行に削除ボタン（`RiCloseLine` アイコン）を持つ（削除ハンドラは `onDelete(subgroupId)` prop で受け取る）。空状態は「サブグループはまだありません」、エラー時はエラーメッセージを表示する。`AddSubgroupSheet` はラジオ選択（単一選択）形式で追加対象グループを選択する。内部で `fetchGroupsForSheet` を 300ms デバウンス付きで呼び出し、現在のグループ自身および既に子グループになっているものを `subgroups` prop で受け取って除外する。追加成功時は `onSuccess()`（`refetch`）→ `onClose()`（`closeSheet()`）を呼び出す。409 競合エラーは「すでに追加済みです」と表示する。`GroupDetailView` がフルページとシート表示の共通描画ロジックを担い、`GroupDetailSheet`・`GroupDetailPage` はそれぞれの表示コンテキスト固有のラッパー。`GroupDetailContent` が Edit ボタン・Delete ボタン・`EditGroupDialog`・`DeleteGroupDialog`・「メンバー追加」ボタン・Subgroups セクション（`SubgroupList` + 「追加」ボタン）を統合する。`GroupDetailContent` の `useGroupDetail` から受け取る `subgroups` を `SubgroupList` と `AddSubgroupSheet` の両方に渡す。「追加」ボタンは `useSheetStack.openSheet` で `AddSubgroupSheet` をシートとして開く。「メンバー追加」ボタンは `useSheetStack.openSheet` で `AddMemberSheet` をシートとして開く。`AddMemberSheet` はマウント時に `clearNonMemberListCache(groupId)` を呼び出して当該グループの非メンバーキャッシュをクリアし、常に最新の非メンバー一覧を取得する。追加成功時は `clearMemberListCache()` → `refetch()`（グループ詳細再取得）→ `onClose()`（`closeSheet()` + `refetch()` 再呼び出し）の順に呼び出し、409 競合エラーは「選択したユーザーはすでにメンバーです」と表示する。`MemberList` は `onRefetch` prop（省略可能）を受け取り、渡された場合のみ複数選択削除 UI（チェックボックス + 「削除」ボタン + 確認 `AlertDialog`）を表示する。ヘッダー行にはネイティブ `<input type="checkbox">` による全選択チェックボックスを配置し、`useRef<HTMLInputElement>` + `useEffect` で `indeterminate` 状態を管理する（`handleSelectAll` で全選択・全解除を切り替え）。削除成功時は `clearMemberListCache()` → `onRefetch()` の順に呼び出してメンバー一覧を更新する |
